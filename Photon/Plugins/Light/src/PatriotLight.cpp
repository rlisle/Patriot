...
void Light::setValue(int value) {
    Log.trace("Light " + _name + " setValue: " + String(value));
...
void Light::startSmoothDimming() {
    if((int)_value == _targetValue){
        Log.trace("Light " + _name + " startSmoothDimming equal");
        return;
    }
    ...
    Log.trace("Light " + _name + " startSmoothDimming target: " + String(_value) + ", increment: " + String(_incrementPerMillisecond));
}
...
void Light::loop()
{
    ...
    if(_incrementPerMillisecond > 0) {
        ...
        if(_currentValue < _targetValue) {
            _value = _targetValue;
            Log.trace("Light "+_name+" loop: down done");
        }
    }
